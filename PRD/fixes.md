...
- [ ] **3. Implement New Correct Guess Flow:**
  ...
  - **3.1. During the Round (Immediate Feedback):**
    ...
    - **Action:** When a player submits a correct guess, the guess input form should be replaced with a clear confirmation message, such as "You got it!" that is the same size as the guess input form so the window does not change size. This state should persist until the next round starts.
