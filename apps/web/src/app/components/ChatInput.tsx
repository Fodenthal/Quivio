  ...
  return (
    <div className="space-y-2" onClick={handleContainerClick}>
      <textarea
        ...
        maxLength={maxLength}
        rows={2}
        className="w-full px-4 py-3 bg-gray-50 dark:bg-black/20 border border-gray-300 dark:border-border-primary rounded-lg text-sm text-text-primary placeholder-text-tertiary focus:outline-none focus:ring-2 focus:ring-accent-primary focus:border-accent-primary disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 resize-none overflow-y-auto"
      />
      
      {message.length > maxLength * 0.8 && (
        <div className="text-right">
          <span className={`text-xs ${
            remainingChars < 20 ? "text-error" : "text-cursor-secondary"
          }`}>
